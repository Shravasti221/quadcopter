...
class flight_controller{
  ...
  float rotor_mean;
  /*
  throttle decides mean speed of all the motors.
  if roll if positive: right tilt
      while(target_ not reached){
        LF++; LL++;
      }
  */
  ...
  void calculate_flight_targets() {
    ...
    rotor_mean = map(rc_throttle(), MIN_RC_THROTTLE, MAX_RC_THROTTLE, 0, 100);
